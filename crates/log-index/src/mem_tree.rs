...
impl MemTreeView {
    ...
    pub fn new_writer(
        ...
        if block_number > 0 {
            ...
            tree_guard.nodes[new_root as usize] =
        ...
        let root = match tree_guard.roots.get_mut(&mv.block_number) {
            Some(r) => r,
            None => panic!("invalid root index"),
        };
        *root = MemTreeRoot { node_index: new_root, block_id };
        drop(tree_guard);
        mv
    }
