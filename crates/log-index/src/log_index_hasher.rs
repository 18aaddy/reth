...
// // Create a provider for node resolution
// let provider = DefaultTrieNodeProvider;

// // Update a leaf node with a new value (similar to set())
// let path = Nibbles::from_nibbles([0x1, 0x2, 0x3]);
// let value = vec![0x01, 0x02, 0x03];
// trie.update_leaf(path.clone(), value, &provider).unwrap();

// // Get the value at a path (similar to get())
// let retrieved_value = trie.get_leaf_value(&path);
